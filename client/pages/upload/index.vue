        ...
        <div class="w-full  mb-[40px] bg-white shadow-lg rounded-md py-6 md:px-10 px-4">
            ...
            <div class="mt-8 md:flex gap-6">
                <label v-if="!fileDisplay" for="fileInput" @drop.prevent="onDrop" @dragover.prevent="" class="
                    ...
                    <input ref="file" type="file" id="fileInput" @input="(e) => onChange(e)" hidden accept=".mp4" />
                </label>
                ...
                <div v-else class="
                    ...
                    <div
                        ...
                        <div class="flex items-center truncate">
                            ...
                            <div class="text-[11px] pl-1 truncate text-ellipsis">{{ file?.name ?? '' }}</div>
                        </div>
...
export default defineComponent({
    ...
    methods: {
        ...
        onChange(event: Event) {
            ...
            if (video) {
                this.file = video;
                preview = window.URL.createObjectURL(video);
            }
            this.fileDisplay = preview ?? '';
        },
        onDrop(event: DragEvent) {
            event.preventDefault();
            let video = event.dataTransfer?.files?.[0];
            let preview
            if (video) {
                this.file = video;
                preview = window.URL.createObjectURL(video);
